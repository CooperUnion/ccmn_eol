...
class Cargo:
    ...
    def __run_cargo(self, source, target, env):
        ...
        env["ENV"]["CARGO_PIO_OPENCAN_NODE"] = env.GetProjectOption("board_can_node")

        env["ENV"]["CARGO_BUILD_TARGET_DIR"] = self.__cargo_target_dir
        env["ENV"]["CARGO_PIO_BUILD_PROJECT_DIR"] = env.subst("$PROJECT_DIR")
        env["ENV"]["CARGO_PIO_BUILD_PROJECT_BUILD_DIR"] = env.subst("$PROJECT_BUILD_DIR")
        ...
        # turn pairs of (define, value) into define=value and then unescape sequences like \\\" to just "
        cppdefines = ' '.join([f'{x[0]}={x[1]}' if type(x) is tuple else x for x in env['CPPDEFINES']])
        cppdefines = bytes(cppdefines, 'utf-8').decode('unicode_escape')

        env["ENV"]["CARGO_PIO_BUILD_CPPDEFINES"] = cppdefines
        env["ENV"]["CARGO_PIO_BUILD_FLAGS"] = ' '.join(env.GetProjectOption("build_flags", default=""))
